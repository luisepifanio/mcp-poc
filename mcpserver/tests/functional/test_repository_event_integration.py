...
async def test_saveMany_integration_conflict_resolution(async_session_local):
    ...
    async with AsyncSessionLocal() as session:
        ...
        original = Event(name="orig", external_uuid=uuid4(), payload={"x": 1}, state=EventState.PENDING)
        ...
        new_ev = Event(name="new", external_uuid=saved.external_uuid, payload={"y": 2}, state=EventState.CREATED)
