...
class Spinner:
    """Lightweight terminal spinnertaken from Kurama orange aesthetics.
    ...
    def __init__(self, text: str, icon: str = "🦉 Nyx | "):
        self.text = text
        self.icon = icon
...
def run_with_spinner(label: str, fn: Callable, *args, **kwargs):
    ...
    s.start()
    try:
        res = fn(*args, **kwargs)  # type: ignore
        s.stop(success=True)
