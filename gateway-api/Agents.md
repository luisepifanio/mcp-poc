|                  |                                                                                                                          |
| ---------------- | ------------------------------------------------------------------------------------------------------------------------ |
| **Project Name** | Gateway Api (gateway-api)                                                                                                |
| **Description**  | Backend Python con Clean Architecture, testing con pytest y CI/CD con GitHub Actions. **Proyecto en monorepo `mcp-poc`** |
| **Domain**       | Backend Service (API Gateway)                                                                                            |
| **Collection**   | Monorepo MCP POC                                                                                                         |
| **Location**     | `/gateway-api` (relativo a raíz del monorepo)                                                                            |
| **Version**      | v0.1.0 (Stable Baseline)                                                                                                 |
...
Este proyecto es **parte del monorepo `mcp-poc`** ubicado en `/Users/luisepifanio/Repos/mcp-poc`.

### Estructura del Monorepo

```
mcp-poc/                          # Raíz del monorepo
├── gateway-api/                  # 📁 Proyecto actual (v0.1.0)
│   ├── app/                      # Código fuente
│   ├── tests/                    # Suite de tests
│   ├── pyproject.toml            # Dependencias del proyecto
│   ├── Agents.md                 # Este documento (proyecto-específico)
│   └── ...
├── [otro-proyecto]/              # Otros proyectos en el monorepo
├── [otro-proyecto]/
├── README.md                      # Guía general del monorepo
└── ...
```

### Interacción con el Monorepo

- **Independencia**: Cada proyecto tiene su propio `pyproject.toml`, entorno virtual y suite de tests
- **Documentación específica**: Este `Agents.md` es específico a `gateway-api` (no clonar de otros proyectos)
- **Herramientas**: Todos los proyectos usan pytest, ruff, mypy con configuración consistente
- **Versionamiento**: Gateway API maneja su propio versionamiento semver (`v0.1.0`)
- **CI/CD**: GitHub Actions ejecuta tests de cada proyecto de forma independiente

### Para Desarrolladores en el Monorepo

1. **Setup del proyecto**: Navega a `gateway-api` y ejecuta `uv sync --group dev`
2. **Testing**: Ejecuta `uv run pytest` dentro del directorio del proyecto
3. **Documentación**: Lee `Agents.md` de cada proyecto para entender su arquitectura y workflow específico
4. **Cambios entre proyectos**: Si tu cambio afecta múltiples proyectos, coordina PRs de forma atómica
...
```bash
# 1. Navegar al proyecto dentro del monorepo
cd /Users/luisepifanio/Repos/mcp-poc/gateway-api
# o si ya estás en mcp-poc/:
cd gateway-api

# 2. Instalar dependencias (crea venv local del proyecto)
# Para builds REPRODUCIBLES (recomendado en CI/CD y deployment):
uv sync --locked --group dev

# O para actualizar lock file (desarrollo local):
uv sync --group dev
...
# 4. Ejecutar servidor (modo desarrollo con auto-reload)
uv run fastapi dev
...
| Package Manager | uv                    | latest        | Dependency management (monorepo)           |
| Framework Web   | FastAPI               | 0.121+        | API REST                                   |
| BD/ORM          | SQLAlchemy + SQLModel | 2.0+ / 0.0.27 | Persistencia (planned v0.2.0)              |
| BD Storage      | SQLite/PostgreSQL     | -             | Dev/Prod                                   |
| Testing         | pytest                | 9.0+          | Tests unitarios/funcionales                |
| Test Coverage   | pytest-cov            | latest        | Coverage reporting (85% gate, 100% ✅)     |
| Linting         | Ruff                  | 0.14+         | **Obligatorio** (0 issues ✅)              |
| Type Checking   | mypy                  | latest        | **Obligatorio** (strict mode, 0 errors ✅) |
| Logging         | logging               | std           | Structured logs                            |
| Async           | asyncio               | std           | Async/await support                        |

### Estructura Carpetas

```
gateway-api/                      # Raíz del proyecto (dentro de monorepo)
├── app/
...
│
├── scripts/
│   └── version.sh               # Auto-versioning script
│
├── Agents.md                     # Este documento (proyecto-específico)
├── README.md
├── CHANGELOG.md
...
- **Si actualizas dependencias**: Siempre commit `uv.lock` junto con cambios en `pyproject.toml`
  ```bash
  uv sync --upgrade  # Actualiza uv.lock
  git add pyproject.toml uv.lock
  git commit -m "chore(deps): update dependencies and lock file"
  ```
...
#### Dependency Management con uv.lock (Build Reproducible)

**Problema resuelto**: Sin `uv.lock`, cada instalación puede resolver diferentes versiones de dependencias, causando inconsistencias entre dev, CI y producción.

**Solución**: `uv.lock` garantiza que TODOS (dev, CI, containers, productivo) usan las MISMAS versiones.

```bash
# ✅ DESARROLLO LOCAL (puedes actualizar dependencias)
uv sync --group dev
# → Crea/actualiza uv.lock con versiones resueltas
# → COMMIT uv.lock a git (¡obligatorio!)

# ✅ CI/CD Y DEPLOYMENTS (reproduicible, no actualiza)
uv sync --locked --group dev
# → USA versiones en uv.lock (fail si falta)
# → NO actualiza uv.lock
# → Garantiza consistencia 100%

# 🔄 ACTUALIZAR DEPENDENCIAS EXPLÍCITAMENTE
uv sync --upgrade --group dev
# → Resuelve nuevas versiones compatibles
# → Actualiza uv.lock
# → COMMIT el nuevo uv.lock

# 🔒 REGENERAR uv.lock desde cero
uv lock --upgrade
# → Útil si uv.lock está corrupto
# → Resuelve desde cero basado en pyproject.toml
```
...
1. **Local (desarrollo)**: `uv sync --group dev` → actualiza lock file
2. **CI/CD**: `uv sync --locked --group dev` → usa lock file exacto
3. **Docker/Prod**: `uv sync --locked` → reproducible en container
4. **Cuando actualizar deps**: `uv sync --upgrade` + commit `uv.lock`

**Verificación rápida**:

```bash
# Ver que uv.lock existe y está sincronizado
ls -lh uv.lock
git status uv.lock  # No debe mostrar cambios no commiteados si está sincronizado

# Ver versiones bloqueadas
uv tree  # Muestra todas las versiones instaladas/bloqueadas
```
